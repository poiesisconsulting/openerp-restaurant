...
class Escpos:
    ...
    def barcode(self, code, bc, width=3, height=20, pos='below', font='a'):
        """ Print Barcode """
        # Align Bar Code()
        ...
        def print_elem(stylestack, serializer, elem, indent=0):
            ...
            if elem.tag in ('p','div','section','article','receipt','header','footer','li','h1','h2','h3','h4','h5'):
                ...
                if 'width' in elem.attrib:
                    width = int(max(2,min(6,math.floor(float(elem.attrib['width'])/100*4 + 2))))
                
                if 'height' in elem.attrib:
                    height = int(max(1,min(255,math.floor(float(elem.attrib['height'])/100*255))))

                if 'text' in elem.attrib and elem.attrib['text'] in ('above','below','both','off'):
                    text = elem.attrib['text']

                if 'font' in elem.attrib and elem.attrib['font'] in ('a','b'):
                    font = elem.attrib['font']

                serializer.start_block(stylestack)
                self.barcode(strclean(elem.text),encoding,width,height,text,font)
                serializer.end_entity()
