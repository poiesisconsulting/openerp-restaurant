...
class hph_server_closing_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(hph_server_closing_report, self).__init__(cr, uid, name, context=context)
        self.total = 0.0
    ...
    def __pos_payment_user__total__(self, form):
        ...
        self.cr.execute ("select sum(pol.price_unit * pol.qty * (1 - (pol.discount) / 100.0)) " \
                         ...
                         "and po.user_id = %s",(tuple(ids),))
        res=self.cr.fetchone()
        res = res and res[0] or 0.0

        return res

report_sxw.report_sxw('report.hph.server.closing', 'pos.order', 'addons/poi_x_hph/report/hph_server_closing_report.rml', parser=hph_server_closing_report,header='internal')