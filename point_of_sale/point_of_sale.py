...
class pos_order(osv.osv):
    ...
    _columns = {
        ...
        'sequence_number': fields.integer('A session-unique sequence number for the order'),
    ...
    def _payment_fields2(self,fields):
        return {
            ...
            'date':   fields.get('payment_date',time.strftime('%Y-%m-%d')),
        }
    ...
    def add_payment(self, cr, uid, order_id, data, context=None):
        ...
        orderis = self.search(cr, uid, [('id','=',order_id)])
        if not orderis or len(orderis)==0:
            return True
        order = self.browse(cr, uid, order_id, context=context)
        args = self._payment_fields2(data)
        args['name']   = order.name + ': ' + (data.get('payment_name', '') or '')
