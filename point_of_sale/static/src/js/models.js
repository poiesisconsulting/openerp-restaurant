...
*/
                    ...
                    return self.fetch(
                        'product.product',
                        ['name', 'list_price',
                         'price','public_categ_id',
                         'taxes_id', 'ean13',
                         'default_code','property_ids',
                         'to_weight', 'uom_id',
                         'uos_id', 'uos_coeff',
                         'mes_type', 'description_sale',
                         'description', 'is_customizable',
                         'course_sequence'],
                        [['sale_ok','=',true],
                    ...
                    return self.fetch(
                        'product.product.property.rel',
                        ['product_id','property_id']
                    );
                }).then(function(property_rel){
                    self.set('property_rel', property_rel);
    ...
    module.Orderline = Backbone.Model.extend({
        initialize: function(attr,options){
            ...
            this.price   = options.product && options.product.price;
            this.quantity = 1;
            this.quantityStr = '1';
            this.discount = 0;
            this.discountStr = '0';
        ...
        get_all_prices: function(){
            ...
            var taxes_ids = product && product.taxes_id;
    ...
    module.Paymentline = Backbone.Model.extend({
        ...
        // // DAO SYSTEMS LA Establece el monto de la propina para la linea de pago.
        set_tips: function(value){
            this.tips = round_di(parseFloat(value) || 0, 2);
            this.trigger('change:tips',this);
        },
        ...
        //// DAO SYSTEMS LA Obtiene el monto de la propina de la linea de pago
        get_tips: function(){
            return this.tips;
        },
    ...
    module.Order = Backbone.Model.extend({
        ...
        getPaidTotal: function() {
            ...
            /* DAO SYSTEMS
            var total =(this.get('paymentLines')).reduce((function(sum, paymentLine) {
                            return sum + paymentLine.get_amount();
                        }), 0);

            //No tomar en cuenta los TIPS, xq se agrega un producto de tips al final.
            total -= this.getTipsTotal();

            return total;
            */
        },
        getTipsTotal: function() {
            //console.log('THIS MODEL.JS', this);
            return (this.get('paymentLines')).reduce((function(sum, paymentLine) {
                //console.log('PAYMENTE LINE', paymentLine);
                return sum + paymentLine.get_tips();
            }), 0);
        },
        getChange: function() {
